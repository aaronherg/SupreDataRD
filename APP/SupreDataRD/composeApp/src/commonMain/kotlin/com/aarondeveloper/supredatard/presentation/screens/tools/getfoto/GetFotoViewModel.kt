...
class GetFotoViewModel(
    private val usuariosRepository: UsuariosRepository,
    private val configuracionRepository: ConfiguracionRepository,
    private val getFotoRepository: GetFotoRepository,
) : ViewModel() {
                                ...
                                is Resource.Success -> {
                                    ...
                                    if (responseUsuarioById.data!!.Validacion == "Exitoso") {
                                        ...
                                        if (!nCedula.isNullOrBlank() && nCedula.length == 11) {
                                            ...
                                            getFotoRepository.getFoto(apikey, nCedula).collect { responseGetFoto ->
                                                when (responseGetFoto) {
                                                    is Resource.Loading -> {
                                                        _uiState.update { it.copy(cargando = true) }
                                                    }
                                                    ...
                                                    is Resource.Success -> {
                                                        _uiState.update { it.copy(cargando = false) }
                                                        if(responseGetFoto.data!!.Validacion.toString() == "Exitoso"){

                                                            val cedula = responseGetFoto.data.cedula

                                                            if(!cedula.isNullOrBlank()){
                                                                _uiState.update { it.copy(nCedula = "", getFotoDto = responseGetFoto.data, encontrado = true) }
                                                            }
                                                            else{
                                                                Notificacion(
                                                                    "Advertencia",
                                                    ...
                                                    is Resource.Error -> {
                                                        ...
                                                        Notificacion(
                                                            "Error",
                                                            "Error: ${responseGetFoto.message.toString()}",
                                                            NotificationType.Error
                                                        )
                                                    }
                                                }
    ...
    fun onBuscarFotoClick() = viewModelScope.launch {
        buscarFoto()
    }
...
data class UiState(
    val getFotoDto: GetFotoDto? = null,
