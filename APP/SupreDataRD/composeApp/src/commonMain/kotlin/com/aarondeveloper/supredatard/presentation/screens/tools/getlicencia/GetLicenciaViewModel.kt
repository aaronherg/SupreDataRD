...
class GetLicenciaViewModel(
    private val usuariosRepository: UsuariosRepository,
    private val configuracionRepository: ConfiguracionRepository,
    private val getLicenciaRepository: GetLicenciaRepository,
) : ViewModel() {
                                ...
                                is Resource.Success -> {
                                    ...
                                    if (responseUsuarioById.data!!.Validacion == "Exitoso") {
                                        ...
                                        if (!nCedula.isNullOrBlank() && nCedula.length == 11) {
                                            ...
                                            getLicenciaRepository.getLicencia(apikey, nCedula).collect { responseGetCiudadano ->
                                                when (responseGetCiudadano) {
                                                    ...
                                                    is Resource.Success -> {
                                                        ...
                                                        if(responseGetCiudadano.data!!.Validacion.toString() == "Exitoso"){
                                                            ...
                                                            val nombres = responseGetCiudadano.data.NombreChofer.toString()

                                                            if(!nombres.isNullOrBlank()){
                                                                _uiState.update { it.copy(nCedula = "", getLicenciaDto = responseGetCiudadano.data, encontrado = true) }
                                                            }
                                                            else{
                                                                Notificacion(
                                                                    "Advertencia",
    ...
    fun onBuscarLicenciaClick() = viewModelScope.launch {
        buscarLicencia()
    }
...
data class UiState(
    val getLicenciaDto: GetLicenciaDto? = null,
